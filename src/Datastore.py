...
class Datastore():
    ...
    def create_price_db(self):
        sessions = self.db.get(self.db_price)
        if sessions:
            print(f"Dictionary is not empty! {sessions}")
        else:
            print("Dictionary is empty!, create databases")
            self.db.dcreate(self.db_price)
            self.db.dump()

    def get_user_data_by_key(self, key:str):
        user_data = self.db.dget(self.db_price, key)
        return user_data

    def addKeyValue(self, key: str, value: str):
        values = (key, value)
        self.db.dadd(self.db_price, values)
        self.db.dump()
    ...
    def check_if_exists(self, session_key:str):
        is_already_present = self.db.dexists(self.db_price, session_key)
    ...
    def updateSession(self, session_id):
        ...
        if exists:
            # Get values
            values = self.db.dget(self.db_price, session_id)
